...
const SETTINGS = {
  ...
  "datasetNote": "generato il 01/09/2026, aggiornato il 16/09/2026"  // testo libero mostrato in fondo alla Home dopo il conteggio domande
};
```
...
- **Ordine delle risposte randomizzato**: ad ogni nuova estrazione della domanda le opzioni di risposta vengono mostrate in un ordine casuale (diverso dall'ordine nel dataset), cosi' non ci si abitua alla posizione ma al contenuto. La correzione e il punteggio non cambiano: solo l'ordine visivo.
- **Categoria per ogni domanda**: ogni domanda mostra un badge con l'area di appartenenza (una delle 7: Fondamenti IA e IA responsabile, Machine Learning, Computer Vision, NLP, Speech, Generative AI, Document Intelligence), visibile durante il quiz, nella schermata "Rivedi risposte" e nell'elenco "Domande in ripasso". La classificazione e' salvata direttamente in `exam-questions.js` (campo `category` di ogni domanda).
- **Statistiche per area** (nello Storico): una nuova card mostra la percentuale di risposte corrette per ciascuna delle 7 aree, ordinate dalla piu' debole alla piu' forte, cosi' e' subito chiaro dove concentrare lo studio (evidenziate in rosso sotto il 60%, in verde dall'85% in su). Calcolate da tutte le domande risposte finora (anche nei test non completati con "Termina"), salvate in `localStorage` (`ai901_category_stats_v1`).
- **Banner "Sei offline"**: appare in alto quando manca la connessione (i dati gia' caricati continuano comunque a funzionare) e scompare automaticamente al ritorno della connessione.
- **Notifica di nuova versione**: se la pagina e' installata come app/aperta online (vedi PWA piu' sotto) e viene pubblicato un aggiornamento, compare un banner "E' disponibile una nuova versione delle domande" con un pulsante per aggiornare subito.
- **Percorso e traguardi** (in Home): una card mostra i giorni consecutivi di allenamento (streak attuale e record), il numero di test completati e il numero di domande "superate" nel ripasso mirato (rispondendo correttamente 5 volte di fila), per invogliare a tornare regolarmente. Calcolati dallo storico gia' salvato, nessun dato aggiuntivo da inserire.
